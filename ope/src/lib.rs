    ...
    pub fn is_allow(&self, list: &[Statement], input: &Request) -> Result<()> {
        ...
        for statement in list.iter() {
            ...
            if let Effect::Deny = statement.effect {
                return Err(Error::Deny(format!("{statement:?}")));
            }
            allowed = true;
        }
        if !allowed {
...
mod tests {
    ...
    use crate::condition::{
        boolean::Boolean, cidr::Cidr, numeric_cmp::NumericCmp, string_cmp::StringCmp,
        string_cmp::StringCmpInner, string_match::StringMatch, time_cmp::TimeCmp,
    ...
    fn is_allow() {
            ...
            conditions: Some(HashMap::from([
                ...
                (
                    ...
                    JsonCondition {
                        ...
                        options: serde_json::value::to_raw_value(&StringMatch {
                            matches: r"^[a-zA-Z][a-zA-Z0-9_#@\\$]{14,254}$".to_owned(),
                        })
                        .unwrap(),
                    },
                ),
        ...
        let p = super::Ope::new(Regexp::new(256).unwrap());
        p.is_allow(
            &sts,
            &Request {
                ...
                context: HashMap::from([
                    ...
                    (
                        "password".to_owned(),
                        serde_json::value::to_raw_value("a12345678901234567").unwrap(),
                    ),
                    (
                        "enable".to_owned(),
                    ...
                    (
                        "login".to_owned(),
                        serde_json::value::to_raw_value("15/01/2023 12:50").unwrap(),
                    ),
                    (
                        "resource".to_owned(),
