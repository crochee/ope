...
impl TimeCmp {
        ...
        match dest_value.format.as_str() {
            ...
            v => match &dest_value.location {
                Some(location) => match location.as_str() {
                    ...
                    "LOCAL" => {
                        if let Ok(src) = NaiveDateTime::parse_from_str(src_value, v) {
                            if let Ok(dest) = NaiveDateTime::parse_from_str(&dest_value.value, v) {
                                match dest_value.symbol.as_str() {
                                    "==" => return src == dest,
                                    "!=" => return src != dest,
                                    ">" => return src > dest,
    ...
    fn cmp_utc(&self, src_value: &str, dest_value: &TimeCmpInner, format: &str) -> bool {
        if let Ok(src) = NaiveDateTime::parse_from_str(src_value, format) {
            if let Ok(dest) = NaiveDateTime::parse_from_str(&dest_value.value, format) {
                match dest_value.symbol.as_str() {
                    "==" => return src == dest,
                    "!=" => return src != dest,
                    ">" => return src > dest,
